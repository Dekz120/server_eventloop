...
Server::Server(size_t port) : Node()
{
    ...
    setFd(fdscr);
    if(fdscr < 0) throw serverExcept("socket()");
    ...
    if(bnd < 0) throw serverExcept("bind()");

    int lsn = listen(fdscr, listener_queue);
    if(lsn < 0 ) throw serverExcept("listen()");
}
Server::Server(Server&& rhs) : Node(std::move(rhs)),
    addr(std::move(rhs.addr)) {};

int Server::handleConnection()
{
    int accept_sd = accept(getFd(), (struct sockaddr*)&addr, (socklen_t*)&addr);
    if(accept_sd < 0 ) throw serverExcept("accept()");
    return(accept_sd);
}
